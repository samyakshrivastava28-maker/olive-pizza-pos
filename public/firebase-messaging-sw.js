/**
 * Olive Pizza POS — Firebase Cloud Messaging Service Worker
 */
...
messaging.onBackgroundMessage(async (payload) => {
  ...
  const notifTitle = payload.notification?.title || data.title || 'Olive Pizza POS';
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((clientList) => {
      ...
      if (clients.openWindow) {
        return clients.openWindow(notifData.url || '/billing');
      }
    })
  );
});
